  ...
  <div class="max-w-lg lg:max-w-3xl xl:max-w-5xl mx-auto">
    <div class="text-center mb-16">
      ...
      <CleanText 
        text={data.title}
        tag="h2"
        className="text-4xl lg:text-5xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-primary-500 to-primary-800 pb-3"
      />
    </div>
            ...
            <ul class="space-y-4 mt-6">
              {#each data.bulletPoints as point}
                <li class="flex items-start space-x-3 group">
                  ...
                  <CleanText 
                    text={point}
                    className="text-lg text-gray-700 transform group-"
                  />
                </li>
              {/each}
            </ul>
      ...
      >
        ...
        {#each data.stats as stat, i}
          ...
          >
            ...
            <CleanText 
              text={stat.label}
              className="stat-label text-sm text-gray-600 opacity-0"
            />
          </div>
        {/each}
      </div>
