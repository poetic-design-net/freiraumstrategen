  ...
  $: if (isContentLoaded) {
    const tl = gsap.timeline({
      ...
      defaults: {
        ease: "power2.out",
        duration: 0.8,
        clearProps: "all" // Ensures clean animation end states
      }
    })
          ...
          <div class="space-y-4">
            <div bind:this={headlineRef}>
              <CleanText 
                text={data.headline}
                tag="h1"
                className="font-heading text-4xl xs:text-5xl md:text-6xl xl:text-7xl font-bold text-white"
              />
            </div>
            {#if data.subheadline}
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-8">
                {#each data.stats as stat}
                  <div class="text-center">
                    ...
                    <CleanText 
                      text={stat.label}
                      className="block text-sm text-gray-200/90 font-medium mt-1"
                    />
                  </div>
                {/each}
              </div>
