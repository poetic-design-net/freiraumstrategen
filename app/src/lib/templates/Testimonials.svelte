<script lang="ts">
  ...
  $: articles = posts.map(post => ({
    img: post.mainImage 
      ...
      : 'https://cdn.sanity.io/images/4gz7oym3/production/f99b18289276ea2555b3ef8b919a2048b9cb250d-379x420.png?w=400',
    title: post.title?.replace(/[\u200B-\u200D\uFEFF]/g, '') || 'Kein Titel',
                  ...
                  <div class="flex items-start">
                    <div class="w-32 h-32 mr-12 flex-shrink-0">
                      <div class="w-full h-full overflow-hidden rounded-full">
                        <img class="object-cover w-full h-full" src={article.img} alt={article.title}>
                      </div>
                    </div>
                    <div class="flex-grow">
                      <img class="block mb-4" src="saturn-assets/images/testimonials/quote-orange.svg" alt="">
                      <h4 class="text-2xl font-semibold text-gray-900 mb-6">{article.title}</h4>
                      <p class="text-gray-500 mb-5">{article.excerpt}</p>
                      <span class="font-semibold text-primary-900">{article.slug}</span>
                    </div>
                  </div>
                </div>
              </div>
